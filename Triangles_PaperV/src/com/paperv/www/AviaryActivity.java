...
public class AviaryActivity extends Activity {
	 ...
	 * correctly you must first get your own API-KEY from
	 * http://aviary.com/android. Then copy the generated API-KEY into a file
	 * called "aviary-credentials.txt" inside your assets folder. Without that
	 * file your Activity would be unable to launch FeatherActivity.
	 */
	...
	private static final String API_KEY = "_Y3e974YNUmCDlGhF7kBIQ";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		imageWidth = (int) ((float) metrics.widthPixels / 1.2);
		imageHeight = (int) ((float) metrics.heightPixels / 1.2);
	...
	/**
	 * Load the image details and pass the result to the
	 * {@link ImageInfoActivity} activity
	 */
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (id == R.id.view_documentation) {
			...
			intent.setData(Uri
					.parse("http://www.aviary.com/android-documentation"));
			startActivity(intent);
		} else if (id == R.id.get_sdk) {

			intent = new Intent(Intent.ACTION_VIEW);
			intent.setData(Uri
					.parse("https://github.com/AviaryInc/Mobile-Feather-SDK-for-Android"));
			startActivity(intent);
		}
	...
	private void onSaveCompleted(final String filepath) {
		...
		if (mSessionId != null) {
			...
			Dialog dialog = new AlertDialog.Builder(this)
					.setTitle("HiRes")
					.setMessage(
							"A low-resolution image was created. Do you want to save the hi-res image too?")
	...
	private boolean setImageURI(final Uri uri, final Bitmap bitmap) {
		...
		// GlobalState.getInstance().image = imageBitmap;
		//
		// if (feather_done)
		// myActivity.finish();
		...
		if (c != null) {
			...
			if (total > 0) {
				if (c.moveToPosition(position)) {
					...
					// you can pass to the Aviary-SDK an uri with a "content://"
					// scheme
					// or an abolute file path like "file:///mnt/..." or just
					// "/mnt/..."

					// using the "content:/" style uri
					// uri = Uri.withAppendedPath(
					// Images.Media.EXTERNAL_CONTENT_URI, String.valueOf( id )
					// );
	...
	private void startFeather(Uri uri) {
		...
		// === MAX SIZE ===
		// Optional
		// you can pass the maximum allowed image size, otherwise feather will
		// determine
		...
		mSessionId = StringUtils
				.getSha256(System.currentTimeMillis() + API_KEY);
		Log.d(LOG_TAG,
	...
	private void processHD(final String session_name) {
		...
		// Now we need to fetch the session information from the content
		// provider
		FeatherContentProvider.SessionsDbColumns.Session session = null;

		Uri sessionUri = FeatherContentProvider.SessionsDbColumns
				.getContentUri(this, session_name);

		// this query will return a cursor with the informations about the given
		// session
		Cursor cursor = getContentResolver().query(sessionUri, null, null,
				null, null);

		if (null != cursor) {
			session = FeatherContentProvider.SessionsDbColumns.Session
					.Create(cursor);
			cursor.close();
		}
		...
		if (null != session) {
			...
			Log.d(LOG_TAG, "session.file_name: " + session.file_name); // original
																		// file,
																		// it is
																		// the
																		// same
			...
			// this query will return the list of actions performed on the
			// original file, during the FeatherActivity session.
			// Now you can apply each action to the hi-res image to replicate
			// the same result on the bigger image
			cursor = getContentResolver().query(actionsUri, null, null, null,
					null);

			if (null != cursor) {
				// If the cursor is valid we will start a new asynctask process
				// to query the cursor
				// and apply all the actions in a queue
				HDAsyncTask task = new HDAsyncTask(
						Uri.parse(session.file_name),
						destination.getAbsolutePath(), session_name);
				task.execute(cursor);
			} else {
	...
	private class HDAsyncTask extends AsyncTask<Cursor, Integer, String> {
		...
		protected String doInBackground(Cursor... params) {
			...
			if (null != cursor) {
				...
				// IMPORTANT NOTE:
				// If in your manifest you're using a different process for the
				// FeatherActivity Activity
				// then you *MUST* call this method before using any of the
				// MoaHD methods, otherwise
		...
		private void saveExif(ExifInterfaceWrapper originalExif, String filename) {
			...
			if (null != newExif && null != originalExif) {
				...
				newExif.setAttribute(ExifInterfaceWrapper.TAG_SOFTWARE,
						"Aviary " + FeatherActivity.SDK_VERSION);
		...
		protected void onPostExecute(String errorString) {
			...
			if (null != errorString) {
				// Toast.makeText( AviaryActivity.this, "There was an error: " +
			...
			new AlertDialog.Builder(AviaryActivity.this)
					.setTitle("File saved")
					.setMessage(
							"File saved in " + dstPath_
									+ ". Do you want to see the HD file?")
		...
		private boolean loadImage(MoaHD moa) throws AviaryExecutionException {
			final String srcPath = IOUtils.getRealFilePath(AviaryActivity.this,
					uri_);
			if (srcPath != null) {
	...
	class DownloadAsync extends AsyncTask<Uri, Void, Bitmap> implements
		...
		protected void onPreExecute() {
			...
			mProgress = new ProgressDialog(AviaryActivity.this);
		...
		protected Bitmap doInBackground(Uri... params) {
			...
			bitmap = DecodeUtils.decode(AviaryActivity.this, mUri, imageWidth,
					imageHeight, sizes);
		...
		protected void onPostExecute(Bitmap result) {
			...
			if (result != null) {
				...
				// Toast.makeText( AviaryActivity.this, "Failed to load image "
