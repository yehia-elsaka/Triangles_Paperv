...
// Fetch user likes IDs
$query = "select * from phpfox_like WHERE user_id = $user_id ORDER BY like_id DESC LIMIT $offset, $limit "; 
$data = mysql_query($query);
while($story_row = mysql_fetch_array( $data )) 
{ 
	$story_id = $story_row['item_id'];	
	$story_query = "select * from phpfox_story WHERE story_id = $story_id "; 
	$story_data = mysql_query($story_query);	
	$row = mysql_fetch_array( $story_data );
	if($row['story_id'] == '' || $row['story_id'] == null)
		continue;	
